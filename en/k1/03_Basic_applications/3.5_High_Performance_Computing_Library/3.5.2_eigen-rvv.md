...
[Eigen](https://eigen.tuxfamily.org) is a high-performance C++ template library for linear algebra, covering matrix and vector operations, numerical solving, and related algorithms. It is widely used in robotics, computer vision, and numerical simulation, and is a core dependency of mainstream frameworks such as ROS, OpenCV, and PCL.

**Key features:**

- **Header-only**: No compilation required — just `#include` to use
- **High performance**: Eliminates temporary variables via expression templates, with automatic SIMD vectorization
- **Feature-rich**: Supports dense matrices, sparse matrices, geometric transforms (rotation matrices, quaternions), linear solvers, and more
- **Cross-platform**: Supports x86, ARM, RISC-V, and other major architectures

## RVV Acceleration

RVV (RISC-V Vector Extension) is the vector extension instruction set for the RISC-V architecture, enabling SIMD acceleration for matrix operations. The SpacemiT X60 8-core processor on K1 natively supports RVV 1.0, including vector sub-extensions such as `zve64d` and `zvfh`.

For the K1 platform, we provide a customized Eigen library `eigen-spacemit` specifically optimized for RVV, which differs significantly from the system default Eigen:

| Comparison | System Eigen (`libeigen3-dev`) | eigen-spacemit |
|:-:|:-:|:-:|
| Install path | `/usr/include/eigen3` | `/opt/eigen-spacemit` |
| RVV acceleration | Not enabled | Optimized for RVV 1.0 |
| Target platform | General (x86/ARM/RISC-V) | Customized for SpacemiT X60 |
| Installation | `apt install libeigen3-dev` | `apt install eigen-spacemit` |

For high-performance linear algebra on K1 (e.g., point cloud processing, SLAM, Kalman filtering), it is recommended to use `eigen-spacemit` to fully utilize the RVV hardware capabilities.

## Usage Example

The following example compares the performance of the system `libeigen3-dev` versus `eigen-spacemit` on 512×512 matrix multiplication.
...
For better RVV compatibility, it is recommended to use gcc 14 and gfortran-14.
...
# OpenMP allows Eigen-supported operators (mainly large matrix multiplication) to use multi-threading.
option(EIGEN_BENCH_ENABLE_OPENMP "Enable OpenMP so Eigen can parallelize supported operators" ON)

# RVV compile flags for g++ 14 and below; g++ 15 currently skips -march/-mabi to avoid incompatibility.
...
# Unified benchmark target creation: pass different Eigen include paths to compare implementations.
...
# SpacemiT Eigen (RVV accelerated)
...
As shown, matrix multiplication performance improves significantly after RVV acceleration (264 ms → 76 ms).
...
Eigen uses its own template implementation for matrix operations by default, while [OpenBLAS](https://www.openblas.net/) is a highly optimized BLAS (Basic Linear Algebra Subprograms) numerical library. By adding the `-DEIGEN_USE_BLAS` macro at compile time, Eigen delegates key operations such as matrix multiplication (`gemm`) to OpenBLAS, leveraging its deeply hardware-tuned implementation.

`openblas-spacemit` is a customized OpenBLAS version for the SpacemiT K1 platform, fully utilizing RVV 1.0 vector extensions and platform-specific features. Compared to using `eigen-spacemit` alone (RVV vectorization), using `openblas-spacemit` as a backend can further improve large-scale matrix multiplication performance.
...
Still using `bench.cpp` from the previous section.
...
Using `openblas-spacemit` as the Eigen backend further improves matrix multiplication performance (76 ms → 28 ms).

### Eight-Core Performance

![](./images/eigen-t4.png)

## Additional Performance Benchmark Data

**Multi-Core Test Notes**

- Multi-core builds require `target_link_libraries(${target_name} PRIVATE OpenMP::OpenMP_CXX)`
- Multi-core test code requires `Eigen::setNbThreads(threads)`, where `threads` should match the number of cores being tested
- All tests below set `threads` equal to the core count

## eigen-spacemit VS libeigen3-dev

- Uses `taskset -c` to pin core count
- Timing strategy: average of 50 runs with 10 warmup iterations
- `no_rvv_avg_ms` represents the performance of the system `libeigen3-dev` package
- `rvv_avg_ms` represents the performance of the `eigen-spacemit` package
...
## eigen-spacemit: With BLAS vs Without BLAS

- Compares the performance difference between using `openblas-spacemit` as the Eigen backend versus not using it
- Uses `taskset -c` to pin core count
- Timing strategy: average of 50 runs with 10 warmup iterations
- `no_blas` represents direct use of the `eigen-spacemit` API
- `blas` represents `eigen-spacemit` with `openblas-spacemit` as the backend
