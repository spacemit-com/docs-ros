...
| :--- | :--- | :--- |
| Start face tracking | "跟着我" (follow me), "跟随我" (follow me), "看着我" (look at me), "人脸跟踪" (face tracking) | Starts face detection and enters face-tracking mode. |
| Start gesture tracking | "跟着手掌" (follow palm), "手势跟随" (gesture follow), "手势跟踪" (gesture tracking), "跟着手" (follow hand) | Starts gesture detection and enters gesture-tracking mode. |
| Stop tracking | "别跟了" (stop following), "停止跟随" (stop following), "不要跟了" (don't follow), "停止手势" (stop gesture) | Stops the current tracking mode and releases NPU resources. |
| Head movement | "左转" (turn left), "右转" (turn right), "抬头" (look up), "低头" (look down), "左歪头" (tilt left), "右歪头" (tilt right) | Controls single-axis head movement (Yaw/Pitch/Roll). |
| Body movement | "身体左转" (body turn left), "身体右转" (body turn right) | Controls rotation about the body's vertical axis. |
| Reset | "回正" (return to center), "回中" (return to center), "复位" (reset) | Returns the full robot to the centered position. |
| Dance performance | "甩头舞" (head-swing dance), "杰克逊" (Jackson), "小鸡啄米" (chicken-peck), "嗯哼歪头" (hmm-tilt) | Performs a preprogrammed dance with synchronized audio. |
| Exit/shut down | "睡觉吧" (go to sleep), "关机" (shut down) | Exits the application. |
