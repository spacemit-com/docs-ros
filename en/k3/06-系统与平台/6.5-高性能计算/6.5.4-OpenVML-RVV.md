...
OpenVML is a mathematical function library for vector data, providing floating-point vector elementary functions, binary operations, reduction operations, and integer vector operations. Its interfaces take batch arrays as input and output, such as `vsSqrt`, `vsAdd`, `vsSum`, `vdExp`, `vs8Dot`, etc., enabling processing of large numbers of contiguous elements at once.

In robotics, perception algorithms, signal processing, machine learning pre/post-processing, and similar scenarios, data streams commonly contain extensive element-wise operations such as absolute value, square root, exponentiation, logarithm, trigonometric functions, vector addition/subtraction/multiplication, and integer quantization data processing. Compared to writing scalar loops manually in business code, using a vector math library delegates these basic operators to platform-optimized implementations, achieving more stable throughput performance.



## RVV Acceleration

The SpacemiT X100 processor on the K3 platform supports RISC-V Vector Extension (RVV 1.0). `openvml-spacemit` is installed at `/opt/openvml-spacemit` and provides OpenVML operator implementations for the K3 platform.

| Item | openvml-spacemit |
| :-: | :-: |
| Installation | `sudo apt install openvml-spacemit` |
| Header Path | `/opt/openvml-spacemit/include` |
| Library Path | `/opt/openvml-spacemit/lib` |
| Main Header | `openvml.h` |
| Main Library | `libopenvml.so` |
| Typical Use | Vector math functions, floating-point element-wise operations, integer vector operations and reduction operations |
...
Alternatively, explicitly specify the installation path:
...
- Using `taskset -c` to bind CPU cores, the script also sets `OMP_NUM_THREADS`.
- `vector_size` is 1048576, `iterations` is 50, `warmup` is 10.
- `avg_ms` represents the average time per operator call, `mevals_per_second` represents throughput in millions of element evaluations per second, `checksum` confirms output participated in computation and results are stable.



### Test Results Summary

This test covers 81 OpenVML operators, including `float32`, `float64`, and `int8` / `int16` / `int32` data types.

### Single-Core Binding Test Results
