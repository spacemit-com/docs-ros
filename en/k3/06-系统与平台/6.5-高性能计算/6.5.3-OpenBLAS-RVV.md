...
[OpenBLAS](https://www.openblas.net/) is an open-source implementation of BLAS (Basic Linear Algebra Subprograms) and parts of LAPACK, providing foundational numerical computing capabilities such as matrix multiplication, vector operations, linear equation solving, and matrix decomposition. OpenBLAS is widely used for linear algebra acceleration in NumPy, SciPy, Eigen, OpenCV, machine learning inference frameworks, and robotics algorithms.

OpenBLAS performance is closely tied to the target CPU architecture, cache parameters, threading model, and core operator implementations. For compute-intensive operators like GEMM (General Matrix Multiply), whether vectorized microkernels are implemented for specific processors directly impacts throughput. The SpacemiT X100 processor on the K3 platform supports RISC-V Vector Extension (RVV 1.0), enabling RVV-optimized libraries to further unlock hardware performance in large-scale matrix multiplication, matrix decomposition, and similar scenarios.



## RVV Acceleration

Two OpenBLAS implementations are available for comparison on the K3 system:

| Item | System OpenBLAS (`libopenblas-pthread-dev`) | SpacemiT OpenBLAS (`openblas-spacemit`) |
| :-: | :-: | :-: |
| Version | OpenBLAS 0.3.32 | OpenBLAS 0.3.32 |
...
| Target Core | `generic` | `x100` |
| Optimization | Generic riscv64 OpenBLAS pthread implementation | RVV-optimized implementation for SpacemiT X100 |
| Typical Use | Compatible with system default BLAS/LAPACK dependencies | High-performance matrix computation and algorithm acceleration on K3 |

This document compares the performance of `libopenblas-pthread-dev` and `openblas-spacemit` through identical BLAS interfaces (such as `cblas_sgemm` / `cblas_dgemm`). Tests switch only the header and library paths, using `OPENBLAS_NUM_THREADS`, `taskset`, and other mechanisms to control thread count and CPU affinity, thereby observing the acceleration effect of the RVV-optimized library on the K3 platform.
...
As shown, the RVV-accelerated sgemm operator performance improved (10.18ms → 8.05ms).
...
- Using `taskset -c` to limit core count
- Single test timing strategy runs 50 iterations and averages, with 10 warmup runs. This test repeats single tests 10 times, averaging metrics and calculating standard deviation
- mean represents the average of 10 tests, sd is standard deviation

### Test Results Summary

This evaluation focuses on the system default `libopenblas-pthread-dev` versus the SpacemiT X100-optimized `openblas-spacemit`, quantifying RVV acceleration benefits under single-core and eight-core conditions. All tests use unified timing and repetition strategies to ensure comparability.

Main conclusions:

- **BLAS Level-3 (matrix-matrix operations)**: `openblas-spacemit` significantly improves core operator performance. Single-core `sgemm` throughput increased from 26.48 GFLOPS to 33.47 GFLOPS (+26.4%), eight-core `sgemm` throughput increased from 150.52 GFLOPS to 183.40 GFLOPS (+22.5%). Operators like `ssymm` and `strmm` generally achieved 15%~23% acceleration; double-precision scenarios (such as `dgemm`) also gained 5%~10% improvements.
- **BLAS Level-1 (vector operations)**: Data movement and scaling operators (such as `scopy`, `dcopy`, `sscal`, `dscal`) achieved 20%~40% performance improvements; reduction operators (such as `sdot`, `ddot`, `nrm2`) showed limited improvements; some operators (such as `sasum`, `dasum`) still slightly underperformed compared to the system library.
- **BLAS Level-2 (matrix-vector operations)**: Overall advantage is not obvious. `sgemv`, `dgemv`, and other operators are about 20%~30% lower compared to the system library, mainly constrained by memory bandwidth and lower computational density.

Overall, `openblas-spacemit` excels in compute-intensive matrix operations and is suitable for robotics perception, SLAM, machine learning inference, and scientific computing workloads that heavily rely on GEMM; for memory-access-dominated vector and matrix-vector operators, benefits should be evaluated based on specific scenarios.
