...
本指南介绍了如何在 **进迭时空的开发板** 上配置 **MIPI 摄像头**，通过 **JDK 的 C++ 接口** 进行图像采集，并对常见问题进行排查与解决。
...
> ⚠️ 若无法确认 CSI 接口编号，
> - 请参考 [MUSE 系列开发板用户手册](https://www.spacemit.com/community/document/info?lang=zh&nodepath=hardware/eco/k1_muse_pi_pro/pi_pro_user_guide.md) 
> - 或尝试 CSI1 ~ CSI3 接口逐个测试。
> - 若均无法识别，可能为硬件故障或未适配的摄像头，后者情况下，建议参考官方文档的 [[Camera 快速点亮导览](https://www.spacemit.com/community/document/info?lang=zh&nodepath=software/SDK/buildroot/camera/camera_development_guide.md)]章节来进行适配操作。

### 配置 Virtuall Camera 模型

1. 将自动生成的摄像头配置文件复制至指定路径 `/root/`，并重命名为 `svivi_cam1.json`：

   ```bash
   sudo cp /tmp/csi1_camera_auto.json /root/svivi_cam1.json
   ```

2. 打开该 JSON 文件 `svivi_cam1.json`，在顶层添加如下字段以启用 Virtual Camera 模式：
