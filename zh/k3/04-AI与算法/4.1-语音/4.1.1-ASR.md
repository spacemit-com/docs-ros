...
Qwen3-ASR 和 Fun-ASR Nano 都需要提交完整音频后再返回结果，不支持模型原生的有状态流式识别。当前只有 SenseVoice 和 Zipformer 接入了本组件的 `Start()` / `SendAudioFrame()` / `Flush()` 流式接口，其中 Zipformer 是原生流式模型。
...
```bash
python -m pip install --upgrade 'spacemit-asr>=1.0.3' \
...
SenseVoice 默认模型路径为 `~/.cache/models/asr/sensevoice/`，包含 `model_quant_optimized.onnx`、`tokens.txt`、`am.mvn`。程序首次运行时会检查模型，缺失时按组件下载逻辑准备。Qwen3-ASR 和 Fun-ASR Nano 需要安装 `llama.cpp-tools-spacemit`，并先启动带媒体后端的 `llama-server`。
...
Python 文件示例使用相同 endpoint，需要 `spacemit-asr>=1.0.3`：
...
| 类型 | 说明 |
| --- | --- |
| `AsrConfig` | 引擎配置。常用字段：`engine`（sensevoice / zipformer / qwen3-asr / funasr）、`model_dir`、`language`（zh/en/ja/ko/yue/auto）、`punctuation`、`sample_rate`、`provider`（cpu / spacemit，默认 spacemit）、`hotwords`、`hotword_boost`、`endpoint`、`model`、`timeout`。可用 `Preset(name)` 创建预置配置。 |
...
| 接口 | 说明 | 参数 | 返回值 |
| --- | --- | --- | --- |
| `AsrEngine(engine, model_dir)` | 通过引擎名快速构造，模型路径为空时使用默认缓存路径。 | `engine`：sensevoice / zipformer / qwen3-asr / funasr；`model_dir`：可选模型目录。 | 引擎实例。 |
| `AsrEngine(AsrConfig)` | 用完整配置构造，可指定语言、标点、provider、热词以及服务 endpoint 等。 | `config`：`AsrConfig` 实例。 | 引擎实例。 |
| `AsrConfig::Preset(name)` | 静态工厂方法，返回指定预设的 `AsrConfig`，可在其上覆写字段。 | `name`：sensevoice / zipformer / qwen3-asr / funasr。 | `AsrConfig`。 |
...
以下示例默认已完成 §2 构建；SenseVoice 模型在 `~/.cache/models/asr/sensevoice/`。Qwen3-ASR `llama-server` 启动方式见 §3.6，Fun-ASR Nano 启动方式见 §3.7。
...
- Qwen3-ASR 和 Fun-ASR Nano 场景先用 `curl http://127.0.0.1:8063/health` 检查 `llama-server` 状态。
...
**测试方法**：在 K3 板卡上使用 `target/k3-com260-omni-agent.json` 构建，运行 `asr_file_demo` 统计日志打印的处理时间和 RTF。Qwen3-ASR 需先按 §3.6 启动 `llama-server`，Fun-ASR Nano 需按 §3.7 启动服务，再使用对应的 `--engine` 和 `--endpoint` 调用。
