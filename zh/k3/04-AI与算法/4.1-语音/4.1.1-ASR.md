...
其中 `-c 4096` 用于限制 llama.cpp 上下文长度，降低 8G 板卡上的 KV cache 内存占用；长音频可按需调大。
...
**测试方法**：在 K3 板卡上使用 `target/k3-com260-omni-agent.json` 构建，运行 `asr_file_demo` 统计日志打印的处理时间和 RTF。Qwen3-ASR 需先按 §3.6 启动 `llama-server`，再用 `--engine qwen3-asr` 和对应 `--endpoint` 调用。
