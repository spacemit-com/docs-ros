...
| Qwen3-ASR 0.6B / 1.7B | `llama-server` 服务 | 通过 chat completions 接口进行离线语音识别。 |
| Fun-ASR Nano | `llama-server` 服务 | 通过 transcription 接口进行离线语音识别。 |
| Gemma4 ASR E2B | `llama-server` 服务 | 原语言转写，或将外语语音翻译为英文。 |
...
SenseVoice 默认模型路径为 `~/.cache/models/asr/sensevoice/`，包含 `model_quant_optimized.onnx`、`tokens.txt`、`am.mvn`。程序首次运行时会检查模型，缺失时按组件下载逻辑准备。Qwen3-ASR、Fun-ASR Nano 和 Gemma4 ASR 需要安装 `llama.cpp-tools-spacemit`，并先启动带媒体后端的 `llama-server`；Gemma4 ASR 要求版本不低于 0.1.7。
...
其中 `001` 至 `004` 用于常规转写和性能测试，`022` 至 `024` 用于 Fun-ASR
多语言转写，`024` 至 `026` 用于 Gemma4 ASR 外语语音转英文。更多资源可在
[ASR 测试音频目录](https://archive.spacemit.com/spacemit-ai/model_zoo/assets/audio/)
按需下载。
...
下面默认启动 Qwen3-ASR 1.7B。使用 0.6B 时，按注释替换 `MODEL_DIR` 和
`MODEL_FILE`。`-c 4096` 用于控制 KV cache 内存占用；8G 内存板卡若仍无法启动，
建议使用 0.6B、配置 swap 或换用更大内存配置。
...
Fun-ASR Nano 使用 `llama-server` 的 OpenAI 兼容 transcription 接口。先停止占用
8063 端口的其他服务，再下载模型：
...
Fun-ASR Nano 当前只支持文件或整段 PCM 的阻塞识别，不支持
`Start()` / `SendAudioFrame()` / `Flush()` 形式的真流式识别。

### 3.8 Gemma4 ASR 转写与英文翻译

Gemma4 ASR 使用 `llama-server` 的 OpenAI 兼容 transcription 接口，同时支持原语言
转写和外语语音转英文。客户端需要 `spacemit-asr>=1.0.4`，服务端需要
`llama.cpp-tools-spacemit>=0.1.7`。
...
`--reasoning off` 用于确保输出 token 预算全部用于最终转写或译文。
`--no-cache-prompt` 避免重复音频性能测试命中 prompt cache。
...
当前 `translate` 任务固定输出英文。Gemma4 ASR 只支持文件或整段 PCM 的阻塞请求，
不支持 `Start()` / `SendAudioFrame()` / `Flush()` 形式的真流式识别。服务启动后的
首个音频请求还会初始化动态 ONNX encoder session，性能评估应使用后续请求。
...
| 类型 | 说明 |
| --- | --- |
| `AsrConfig` | 引擎配置。常用字段：`engine`（sensevoice / zipformer / qwen3-asr / funasr / gemma4-asr）、`model_dir`、`language`（zh/en/ja/ko/yue/auto）、`punctuation`、`sample_rate`、`provider`（cpu / spacemit，通用默认值为 spacemit）、`hotwords`、`hotword_boost`、`endpoint`、`model`、`timeout`、`task`（transcribe / translate，默认 transcribe；translate 仅 Gemma4 ASR 支持）。可用 `Preset(name)` 创建预置配置。 |
...
| `AsrEngine(AsrConfig)` | 用完整配置构造，可指定语言、标点、provider、热词以及服务 endpoint 等。 | `config`：`AsrConfig` 实例。 | 引擎实例。 |
...
| Fun-ASR 调用失败 | `llama-server` 未启动、模型 alias 不一致或 endpoint 错误 | 确认服务使用 `--alias funasr`，并检查 endpoint 是否以 `/v1/audio/transcriptions` 结尾。 |
| Gemma4 ASR 返回空文本或耗时异常 | `llama-server` 版本低于 0.1.7，或启动时未关闭 reasoning | 使用 `llama.cpp-tools-spacemit>=0.1.7`，并添加 `--reasoning off`。 |
...
使用 `001_zh_daily_weather.wav` 和 `004_zh_selling_sausages.wav` 连续识别
3 轮。总音频时长为 47.331 秒。

| 模型 | LLM 线程数 | AudioEncoder 线程数 | 总处理时间 | RTF |
| --- | --- | --- | --- | --- |
| Qwen3-ASR 0.6B | 4 | 4 | 10.257 秒 | 0.217 |
| Qwen3-ASR 0.6B | 8 | 4 | 7.977 秒 | 0.169 |
| Qwen3-ASR 1.7B | 4 | 4 | 19.883 秒 | 0.420 |
| Qwen3-ASR 1.7B | 8 | 4 | 15.572 秒 | 0.329 |

### Fun-ASR Nano (Q4_K_M, llama-server)

测试环境为 `llama.cpp-tools-spacemit 0.1.6`、
`spacemit-onnxruntime 2.0.5`、LLM 4 线程和 AudioEncoder 4 线程。服务启动参数为
`-t 4 -tb 4 -c 4096 --warmup --jinja`，未使用 CPU 绑核。

| 测试文件 | 语言 | 音频时长 | 第 1 轮处理时间 / RTF | 第 2 轮处理时间 / RTF |
| --- | --- | --- | --- | --- |
| 004_zh_selling_sausages.wav | 中文 | 14.158 秒 | 3.804 秒 / 0.269 | 3.793 秒 / 0.268 |
| 官方 en.mp3 | 英文 | 7.176 秒 | 1.555 秒 / 0.217 | 1.561 秒 / 0.217 |
| 官方 ja.mp3 | 日文 | 7.224 秒 | 1.706 秒 / 0.236 | 1.709 秒 / 0.236 |
| **每轮合计** | - | **28.558 秒** | **7.065 秒 / 0.247** | **7.063 秒 / 0.247** |

以上 RTF 包含文件读取、音频转换、HTTP 传输和模型推理，不是单独的模型
kernel 耗时。Qwen3-ASR 与 Fun-ASR 使用的测试集不同，不能只根据 RTF 比较识别质量。

### Gemma4 ASR (Q4_0, llama-server 0.1.7)

服务使用 `-t 8 -tb 8 -c 4096 --warmup --jinja --reasoning off` 和
`--no-cache-prompt`。以下数据由干净构建的 `asr_file_demo` 在 K3 上端到端测得。

| 任务 | 测试文件 | 音频时长 | 处理时间 | RTF | 结果 |
| --- | --- | --- | --- | --- | --- |
...
RTF 包含 SDK 文件读取、音频转换、HTTP multipart 传输和 `llama-server` 推理时间。
服务启动后的首个音频请求还会初始化动态 ONNX encoder session；本次日语样例首请求
为 12567 ms（RTF 1.740），未计入表中稳态数据。

日语、韩语和粤语样例在同一预热服务中连续测试 2 轮，总音频时长 34.104 秒，
总处理时间 19.703 秒，端到端 RTF 为 0.578。该结果用于验证接口、输出稳定性和性能，
不替代带参考译文的翻译质量评测。
