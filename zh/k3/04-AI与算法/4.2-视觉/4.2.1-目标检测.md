...
- 规格或特性：
  - 支持模型：YOLOv5（n/s）、YOLOv8（n/s/m）、YOLOv11（n/s/m）
  - 输入尺寸：`[1, 3, 640, 640]`
  - 量化类型：int8
...
- C++ 依赖：OpenCV 4（core、imgproc、highgui）、spacemit-ort、yaml-cpp、Eigen3
- Python 依赖：NumPy、OpenCV 4.12+、ONNX Runtime、Pillow、scipy、pydantic、spacemit-ort
- 模型权重存放路径：`~/.cache/models/vision/yolov8/`、`~/.cache/models/vision/yolov5/`、`~/.cache/models/vision/yolov11/`
- 标签文件：`assets/labels/coco.txt`（COCO 80 类）
- 硬件：SpacemiT K3 开发板
...
编译产物位于 `build/examples/` 目录下，各模型对应可执行文件如 `yolov8`、`yolov5`、`yolov11`。
...
```bash
cd model-zoo-vision
./build/examples/yolov8 examples/yolov8/config/yolov8.yaml
```

**步骤 3**（可选）：使用摄像头实时检测

```bash
./build/examples/yolov8 examples/yolov8/config/yolov8.yaml --use-camera --camera-id 0
```
...
# C++
./build/examples/yolov5 examples/yolov5/config/yolov5.yaml
```
...
# C++
./build/examples/yolov11 examples/yolov11/config/yolov11.yaml
```
...
| 类型 | 说明 |
| --- | --- |
| VisionServiceResult | 检测结果，包含边界框坐标（x1, y1, x2, y2）、置信度（score）、类别 ID（label）。 |
...
| 接口 | 说明 | 参数 | 返回值 |
| --- | --- | --- | --- |
| ModelFactory.create | 从 YAML 配置文件创建检测器实例 | config_path：YAML 配置文件路径 | 检测器对象 |
...
| 接口 | 说明 | 参数 | 返回值 |
| --- | --- | --- | --- |
| InferImage | 对单张图像进行目标检测 | image_path：图像文件路径；results：输出结果向量 | bool（成功/失败） |
| InferImage | 对 cv::Mat 图像进行目标检测 | image：OpenCV Mat 对象；results：输出结果向量 | bool（成功/失败） |
| Draw | 在图像上绘制检测结果（边界框、标签、置信度） | input：输入图像；output：输出图像 | void |
...
| 接口 | 说明 | 参数 | 返回值 |
| --- | --- | --- | --- |
| detect | 对图像进行目标检测 | image_path：图像文件路径或 numpy 数组 | 检测结果列表 |
...
# 1. 创建检测器
detector = ModelFactory.create("examples/yolov8/config/yolov8.yaml")
...
output = detector.draw(image, results)
...
**参考 demo 路径**：
- `examples/yolov8/`、`examples/yolov5/`、`examples/yolov11/`
...
| 推理速度慢 | 线程数设置不当或使用了 CPU provider | 确认 YAML 中 `providers` 为 `SpaceMITExecutionProvider`，`num_threads` 设为 4 |

## 附录：性能与测试数据

### K3 平台

| 具体模型 | 输入大小 | 数据类型 | 帧率 (4核) |
| --- | --- | --- | --- |
| yolov5n | [1,3,640,640] | int8 | 56.9 |
| yolov5s | [1,3,640,640] | int8 | 37.6 |
| yolov8n | [1,3,640,640] | int8 | 54.6 |
| yolov8s | [1,3,640,640] | int8 | 35.0 |
| yolov8m | [1,3,640,640] | int8 | 19.1 |
| yolo11n | [1,3,640,640] | int8 | 41.4 |
| yolo11s | [1,3,640,640] | int8 | 28.3 |
| yolo11m | [1,3,640,640] | int8 | 14.4 |

**测试方法**：使用 `onnxruntime_perf_test` 工具，以 YOLOv8n 为例：
