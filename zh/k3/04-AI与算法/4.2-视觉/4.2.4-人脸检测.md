...
- 模型权重存放路径：`~/.cache/models/vision/yolov5-face/`
- 硬件：SpacemiT K3 开发板
...
```bash
./build/examples/yolov5-face examples/yolov5-face/config/yolov5-face.yaml
```

**步骤 3**（可选）：使用摄像头实时人脸检测

```bash
./build/examples/yolov5-face examples/yolov5-face/config/yolov5-face.yaml --use-camera --camera-id 0
```
