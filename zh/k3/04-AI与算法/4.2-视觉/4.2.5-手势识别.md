...
| 接口 | 说明 | 参数 | 返回值 |
| --- | --- | --- | --- |
| ModelFactory.create_model | 创建手势识别器实例 | model_name：模型名（config 文件名去扩展名）；config_dir：配置文件所在目录 | 手势识别器对象 |
...
| 接口 | 说明 | 参数 | 返回值 |
| --- | --- | --- | --- |
| infer | 对图像进行手势识别 | image：numpy 数组（cv2.imread 读入的图像） | 手势识别结果列表（每项为 dict，键 bbox/class_id/confidence） |
| draw_detections | 在图像上绘制识别结果（来自 `common.python.drawing`） | image、boxes、classes、scores、labels | 绘制后的图像 |
...
# 1. 创建手势识别器
config_path = Path("examples/yolov5_gesture/config/yolov5_gesture.yaml")
detector = ModelFactory().create_model("yolov5_gesture", config_dir=config_path.parent)
...
# 3. 处理结果
print(f"Detected {len(detections)} gesture(s):")
boxes, classes, scores = [], [], []
for d in detections:
    x1, y1, x2, y2 = map(int, d["bbox"])
    print(f"  Gesture class {d['class_id']}, Score: {d['confidence']:.4f}, "
          f"Box: [{x1},{y1},{x2},{y2}]")
...
# 4. 绘制结果
output = draw_detections(image, np.array(boxes), np.array(classes), np.array(scores), None)
...
while True:
    ...
    for d in detections:
        ...
        classes.append(d["class_id"])
        scores.append(d["confidence"])
    output = draw_detections(frame, np.array(boxes), np.array(classes), np.array(scores), None)
