...
| 接口 | 说明 | 参数 | 返回值 |
| --- | --- | --- | --- |
| ModelFactory().create_model | 从配置目录创建表情识别器实例 | model_name：模型名（如 "emotion"，与 config 文件名 stem 一致）；config_dir：配置目录路径 | 表情识别器对象 |
...
| 接口 | 说明 | 参数 | 返回值 |
| --- | --- | --- | --- |
| infer | 对人脸裁剪图进行表情识别（输入必须是 numpy 数组，不接受文件路径） | face_images：单张或多张人脸裁剪图（numpy 数组） | 表情识别结果列表（每项含 `emotion` 类别 ID） |
...
# 1. 创建表情识别器
config_path = Path("examples/emotion/config/emotion.yaml")
emotion_classifier = ModelFactory().create_model("emotion", config_dir=config_path.parent)

# 2. 读取人脸裁剪图（infer() 只接受 numpy 数组，不接受文件路径）
face_image = cv2.imread("face.jpg")

# 3. 执行推理
result = emotion_classifier.infer(face_image)

# 4. 处理结果
print(f"Emotion class: {result[0]['emotion']}")
```
