...
| 接口 | 说明 | 参数 | 返回值 |
| --- | --- | --- | --- |
| ModelFactory().create_model | 从模型名称与配置目录创建人脸识别器实例 | model_name：模型名称（如 "arcface"）；config_dir：配置文件目录 | 人脸识别器对象 |
...
| 接口 | 说明 | 参数 | 返回值 |
| --- | --- | --- | --- |
| infer | 提取人脸特征向量（返回归一化 embedding） | image：BGR 格式的 numpy 数组 | 特征向量（numpy 数组） |
| compute_similarity | 计算两个特征向量的余弦相似度 | embedding1：特征向量 A；embedding2：特征向量 B | float（相似度，范围 -1 到 1） |
...
# 1. 创建人脸识别器
config_path = Path("examples/arcface/config/arcface.yaml")
recognizer = ModelFactory().create_model("arcface", config_dir=config_path.parent)

# 2. 提取人脸特征（infer 返回归一化 embedding）
embedding_a = recognizer.infer(cv2.imread("face_a.jpg"))
embedding_b = recognizer.infer(cv2.imread("face_b.jpg"))
...
for name in names:
    embedding = recognizer.infer(cv2.imread(f"database/{name}.jpg"))
    face_database.append(embedding)

# 2. 提取待识别人脸的特征
query_embedding = recognizer.infer(cv2.imread("query.jpg"))
