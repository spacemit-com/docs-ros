...
- 模型权重存放路径：`~/.cache/models/vision/yolov8_seg/`
...
```bash
./build/examples/yolov8_seg examples/yolov8_seg/config/yolov8_seg.yaml
```

**步骤 3**（可选）：使用摄像头实时分割

```bash
./build/examples/yolov8_seg examples/yolov8_seg/config/yolov8_seg.yaml --use-camera --camera-id 0
```
