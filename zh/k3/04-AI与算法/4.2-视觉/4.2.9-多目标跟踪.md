...
- 模型权重存放路径：
  - ByteTrack：`~/.cache/models/vision/yolov8/yolov8n.q.onnx`
  - OC-SORT：`~/.cache/models/vision/ocsort/yolov8n.q.onnx`
- 测试视频：`~/.cache/assets/video/003_palace.mp4`
- 硬件：SpacemiT K3 开发板
...
./build/examples/bytetrack examples/bytetrack/config/bytetrack.yaml --use-camera --camera-id 0

# C++ OC-SORT（可选摄像头设备号，默认 0）
./build/examples/ocsort examples/ocsort/config/ocsort.yaml --use-camera --camera-id 0
```
...
```bash
# ByteTrack
./build/examples/bytetrack examples/bytetrack/config/bytetrack.yaml

# OC-SORT
./build/examples/ocsort examples/ocsort/config/ocsort.yaml
```
