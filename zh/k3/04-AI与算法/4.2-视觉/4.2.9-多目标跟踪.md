...
| 接口 | 说明 | 参数 | 返回值 |
| --- | --- | --- | --- |
| ModelFactory.create_model | 从 YAML 配置创建跟踪器实例 | model_name：模型名（config 文件名去扩展名）；config_dir：配置目录 | 跟踪器对象 |
...
| 接口 | 说明 | 参数 | 返回值 |
| --- | --- | --- | --- |
| infer | 对单帧图像进行检测和跟踪 | frame：numpy 数组（BGR 图像） | 跟踪结果列表 List[Dict]，每项含 track_id、score、class_id、tlbr（[x1, y1, x2, y2]） |
| draw_results | 在图像上绘制跟踪结果 | image：输入图像；results：跟踪结果；labels：可选类别标签 | 绘制后的图像 |
...
# 1. 创建跟踪器（model_name 为 config 文件名去扩展名）
config_path = Path("examples/bytetrack/config/bytetrack.yaml")
tracker = ModelFactory().create_model("bytetrack", config_dir=config_path.parent)
...
while True:
    ...
    # 3. 执行跟踪
    results = tracker.infer(frame)

    # 4. 处理结果（每项含 track_id、score、class_id、tlbr）
    print(f"Tracked objects: {len(results)}")
    for r in results:
        x1, y1, x2, y2 = r['tlbr']
        print(f"  Track ID: {r['track_id']}, Class: {r['class_id']}, "
              f"Score: {r['score']:.4f}, "
