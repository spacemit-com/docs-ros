...
- 模型权重存放路径：`~/.cache/models/vision/yolov8_pose/`
- 硬件：SpacemiT K3 开发板
...
```bash
./build/examples/yolov8_pose examples/yolov8_pose/config/yolov8_pose.yaml
```

**步骤 3**（可选）：使用摄像头实时姿态估计

```bash
./build/examples/yolov8_pose examples/yolov8_pose/config/yolov8_pose.yaml --use-camera --camera-id 0
```
