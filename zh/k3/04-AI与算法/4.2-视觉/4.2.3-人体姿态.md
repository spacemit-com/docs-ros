...
| 接口 | 说明 | 参数 | 返回值 |
| --- | --- | --- | --- |
| ModelFactory().create_model | 从 YAML 配置目录创建姿态估计器实例 | model_name：模型名（yaml 文件名）；config_dir：配置目录 | 姿态估计器对象 |
...
| 接口 | 说明 | 参数 | 返回值 |
| --- | --- | --- | --- |
| infer | 对图像进行姿态估计 | image：numpy 数组（cv2.imread 读入） | 结果列表，每个元素为含 box/score/keypoints 的 dict |
| draw_keypoints | 在图像上绘制姿态结果（来自 `common.python.drawing`） | image：输入图像；detections：含 box/keypoints 的 dict 列表 | 绘制后的图像 |
...
# 1. 创建姿态估计器（指定 yaml 所在目录）
config_path = Path("examples/yolov8_pose/config/yolov8_pose.yaml")
pose_detector = ModelFactory().create_model("yolov8_pose", config_dir=config_path.parent)

# 2. 执行推理
image = cv2.imread("test.jpg")
results = pose_detector.infer(image)

# 3. 处理结果（每个 result 为含 box/score/keypoints 的 dict）
...
for r in results:
    ...
    print(f"  Person - Score: {confidence:.4f}, Box: [{x1},{y1},{x2},{y2}]")

    # 访问关键点（每个为 [x, y, visibility]）
    keypoints = r.get("keypoints", [])
    print(f"  Keypoints: {len(keypoints)} points")
    for i, kp in enumerate(keypoints):
        print(f"    [{i}] ({kp[0]:.1f},{kp[1]:.1f}) "
              f"visibility: {kp[2]:.2f}")

    detections.append({"box": [x1, y1, x2, y2], "keypoints": keypoints})

# 4. 绘制结果
output = draw_keypoints(image, detections)
...
while True:
    ...
    results = pose_detector.infer(frame)
    detections = []
    for r in results:
        if "box" in r:
            x1, y1, x2, y2 = map(int, r["box"])
            detections.append({"box": [x1, y1, x2, y2], "keypoints": r.get("keypoints", [])})
    output = draw_keypoints(frame, detections)
