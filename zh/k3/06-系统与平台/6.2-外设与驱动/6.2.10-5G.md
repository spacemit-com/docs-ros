...
- `cmake` 报错 `udhcpc not found`：说明当前构建机或目标环境缺少 `udhcpc`。  对于交叉编译后在板端运行，需要安装udhcpc以获取ip。
- `AT port auto-detect failed`：通常是模组未完成 USB 枚举、没有 `bInterfaceProtocol=12` 的 AT interface，或串口节点权限不足。  
- `RNDIS mode detected`：当前组件只支持 ECM/NCM，需要先把模组切到对应 USB 网络模式。 切换方式如下：
```
#安装串口软件：apt install microcom
microcom -s 9600 -p /dev/ttyUSB0
#开启串口后，直接输入以下指令切换
  AT
  AT+UNETMODECFG?
  AT+UNETMODECFG=0
...
#切换后，检查是否有切换
cat /sys/class/net/*/device/interface
#预期输出
