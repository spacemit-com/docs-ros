...
![RiVision 系统架构](./assets/rivision_arch.png)
...
![视频接入流](./assets/rivision_flow_ingest.png)

### 4.2 YOLO 检测流

![YOLO 检测流](./assets/rivision_flow_yolo.png)

### 4.3 VLM 场景理解流

![VLM 场景理解流](./assets/rivision_flow_vlm.png)

### 4.4 语义搜索流

![语义搜索流](./assets/rivision_flow_search.png)
...
![部署拓扑](./assets/rivision_deploy.png)
...
| GET | `/api/vlm/results` | 分页查询历史结果（实现中，参数预留 `limit` / `offset` / `camera_id`） |
| GET | `/api/vlm/results/:id` | 获取单条结果 |
| DELETE | `/api/vlm/results/:id` | 删除结果 |
| GET | `/api/vlm/stats` | VLM 调用统计 |
| GET | `/api/vlm/status` | VLM 触发器运行状态 |

#### 11.2.3 VLM 语义搜索 `/api/vlm/search`

文本/图像语义检索独立路由组，请求体均为 JSON（区别于 `/api/vlm/results` 的列表查询）。

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| POST | `/api/vlm/search` | 文本搜索，请求体 `{"query":"<text>", "camera_id":"<id>", "limit":<N>}` |
| POST | `/api/vlm/search/image` | 以图搜图，请求体含 `caption` / `tags` / `image_embedding` / `image_base64` / `task_id` 等可选字段 |
| POST | `/api/vlm/search/hybrid` | 混合搜索（文本 + 图像向量加权），请求体含 `HybridSearchInput` 字段集 + 可选 `image_base64` |
| GET | `/api/vlm/search/stats` | 索引总数与 Provider 状态 |
| GET | `/api/vlm/search/provider/status` | Embed Provider 状态（EmbedProvider / RemoteProvider / HashProvider 链） |
| POST | `/api/vlm/search/reindex` | 从网关历史导入并更新本地 SQLite 语义索引 |
| POST | `/api/vlm/search/backfill` | 回填缺失的图像 / 文本 Embedding |

#### 11.2.4 YOLO 控制 `/api/yolo`
...
| GET | `/api/v1/stats` | 总览 |
| GET | `/api/v1/stats/overview` | 全局指标 |
...
# 2. 通过 rivision_cli 进行语义搜索（POST /api/vlm/search，混合评分排序）
curl -s -X POST http://localhost:8080/api/vlm/search \
  -H 'Content-Type: application/json' \
  -d '{"query":"红色轿车","limit":10}' | jq '.items'
```
