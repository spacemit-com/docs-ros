...
x86_64 端编译 `simulation/mujoco` 或 `model_zoo/rl` 时，CMake 按以下顺序查找，命中即用：

1. `-DMUJOCO_DIR / -DONNXRUNTIME_DIR=...` 编译参数
2. 同名环境变量
3. 系统路径（`/usr/local`、`/usr`、`/opt/mujoco` 等）
4. 缓存路径 `~/.cache/thirdparty/<name>/<version>/`
5. 上述均未命中：从官方 release 拉取 prebuilt tarball 解压到第 4 项缓存路径
...
> `export MUJOCO_DIR=...` / `export ONNXRUNTIME_DIR=...` 指向预先下载好的目录。其他版本见
...
```bash
# 安装 SpacemiT 定制版 onnxruntime：
sudo apt install -y spacemit-onnxruntime python3-onnx 

# 若同时安装无加速版 onnxruntime，运行时会冲突，需卸载:  
...
`1`~`9` 可在 `POWER_OFF` / `DAMP` 状态预选策略，无需重启进程。策略序号由 YAML 中 `rl_policy.onnx_infer.policies` 下的策略列表顺序决定（从 1 开始）。进入 `ZERO` 后策略锁定，此后发切换指令会被忽略——ZERO 的目标位置/kp/kd 取自当前策略的 `rl_default_pos / kp / kd`，切换会同步重建 ZERO 状态。

若目标策略在 yaml 中配置了 `prerequisite: { policy, duration }`（如 dance / kungfu 配 `stand` 2 秒），切换到该策略时 behavior_manager 会自动先切到前置策略运行指定时长，再自动切到目标策略。

下图为在 DAMP 状态下切换到 dance 策略的示例：

![切换RL策略示例](assets/hmi_switch_policy.png)

### 4.3 运行效果

MuJoCo 窗口打开后机器人悬挂在空中；按 `o` → `z` → `r` 依次切换状态，进入 RL 后机器人站立并响应速度指令。

`driver_runtime` 根据 control 端发来的 `ControlMode` 自动管理悬挂保护：进入 `RL` 状态时取消悬挂、回到 `POWER_OFF` 时重新启用。MuJoCo 窗口内 `F` 键仍可在 `POWER_OFF` / `DAMP` / `ZERO` 非 RL 状态下手动切换悬挂；`↑`/`↓` 调节悬挂高度，`R` 重置悬挂高度。

> **提示**：跑完一个策略后通过 `f` → `o` → `z` 链路切换到 ZERO 准备下一次 RL 时，应在 MuJoCo 窗口里观察机器人此时的姿态。如果机器人此前摔倒、瘫软在地或姿态偏离当前策略 `rl_default_pos` 较远，可在悬挂保护启用的状态下用 `↑`/`↓` 调节悬挂高度，让机器人在 ZERO 状态下尽量回到接近初始姿态再切 RL，避免 ZERO→RL 瞬间因预设位置与实际位置差距过大造成关节冲击或 RL 控制失稳。
...
1. PC 已具备 onnxruntime（首次 `mm` 会自动拉到 `~/.cache/thirdparty/onnxruntime/`，或预装到 `/usr/local`）
...
| 切到 dance / kungfu 后机器人停在 stand 不动作 | 这两个策略配置了 `prerequisite: stand`，behavior_manager 会先跑前置策略 `stand` 满 `duration` 秒（默认 2s）再自动切目标策略；观察 control 终端是否有 `[BehaviorManager] 策略链调度: ...` 日志，等满后会自动切换。若一直停留，检查 yaml 中 `rl_policy.onnx_infer.policies.<目标策略>.prerequisite` 是否配置正确，以及前置策略 `stand` 是否存在并能正常加载 |
