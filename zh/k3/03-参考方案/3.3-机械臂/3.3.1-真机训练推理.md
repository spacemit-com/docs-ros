...
- **MCP 工具封装**：将 LeRobot 抓取流程封装为 C++ 原生应用，通过 `mlink device → mlink gateway → MCP` 链路注册为 MCP 工具；
- **Hermes 自然语言控制**：将 MCP 绑定到 Hermes，并使用 Hermes 以自然语言方式调用 `lerobot.pick_cube` 工具，触发 SO101 机械臂执行抓取任务。
...
```bash
# pyenv 在初始化时会用 readlink 解析自身的符号链接，容易引发 `invalid argument` 报错
# 将 /usr/bin/readlink 的软链接指向已安装的 GNU 版本 /usr/bin/gnureadlink
sudo ln -sf /usr/bin/gnureadlink /usr/bin/readlink
...
1. 如果是组装好的机械臂，一般已完成舵机标定，直接校准即可。校准之前，接入主导臂、随从臂的电源和 USB 口，查询设备端口：
...
> 4. 主导臂主要用于遥操作和数采，推荐将其接入 PC 进行校准、遥操和数采。
...
4. 可视化遥操作，确认 top 和 wrist 两路视觉输入质量：
...
1. **可选**：登录 Hugging Face，便于上传数据集和模型。如需登录请先前往 [https://huggingface.co/](https://huggingface.co/) 创建 huggingface 帐号：
...
也可直接使用笔者采集的 [数据集](https://archive.spacemit.com/spacemit-ai/model_zoo/vla/act/datasets/record-green-cube.tar.gz)，确保场景一致可复用于训练。数据集下载之后解压放至 `~/.cache/huggingface/lerobot/${HF_USER}/` 目录。
...
1. **可选**：登录 wandb 观察训练曲线，登录之前需要先前往 [https://wandb.ai/site/](https://wandb.ai/site/) 创建 wandb 帐号：
...
1. 训练完成后，将模型检查点复制到开发板 `lerobot` 工作空间。
...
也可以下载笔者已训练好的 [模型权重](https://archive.spacemit.com/spacemit-ai/model_zoo/vla/act/so101_act_pick_green_cube_amp.tar.gz)，下载之后放置于 `lerobot/outputs/train/` 目录下。
...
> [!NOTE]
>
> 1. 请根据实际模型路径替换 `--policy.path`。
> 2. 多轮测试时，每轮测试前请先删除上次测试保留的评估数据集避免冲突：`rm -rf ~/.cache/huggingface/lerobot/${HF_USER}/eval_act`。
...
- 可根据前文训练得到 ACT 策略模型；
- 也可下载笔者已训练好的 [模型权重](https://archive.spacemit.com/spacemit-ai/model_zoo/vla/act/so101_act_pick_green_cube_amp.tar.gz)；
...
1. 使用 Python 3.12 创建 FP16 专用虚拟环境：

```bash
~/.lerobot-venv/bin/python -m venv ~/.lerobot-venv-torh2.8
source ~/.lerobot-venv-torh2.8/bin/activate
python -V
```

该命令仅使用默认环境的 Python 3.12 解释器创建新环境，不会继承原环境的 Torch 和 site-packages。

2. Torch 2.8 依赖 `libopenblas.so.0` 和 `libomp.so.5`，直接安装对应系统库：
...
3. 安装 Torch 和 SpineDNN 插件：

```bash
cd ~/spacemit_robot/components/thirdparty/lerobot
source ~/.lerobot-venv-torh2.8/bin/activate

python -m pip install torch==v2.8.0 \
  spine-torch-extension==v2.8.0+spacemit.0
...
上述直链来自 SpaceMIT PyPI，并通过 SHA256 固定已验证的 K3 wheel。不要改为仅安装 `torch==2.8.0`，否则 pip 可能选择同一源中的其他 Torch 2.8 构建。

4. 验证 FP16 环境。无需应用补丁，专用启动脚本会在当前进程中加载 ACT FP16 增量源码：
...
- 将 ACT 权重转换为 `torch.float16`；
- 保持原有 `config.json` 结构，FP16 精度由专用运行入口在进程内设置；
- 保留原模型目录中的预处理、后处理、归一化统计和训练配置文件；
- 不修改原 FP32 模型目录。
...
2. 删除同名的历史本地评估数据集，避免 LeRobot 拒绝重复创建：
...
- `--policy.path`：指向转换后的 FP16 模型目录；
- `examples/spine-torch-deploy/run_lerobot_fp16_a100x8.sh`：设置 AI Core/SpineDNN 环境，并将 `lerobot-record` 切换到仅当前进程生效的 FP16 入口。
...
- `path=full`：一次完整真机推理，包含 observation 转换、输入预处理、ACT `select_action` 和输出后处理；不包含相机取帧、数据集写入和机械臂通信；
- `path=cached`：直接读取 ACT 已生成的 action chunk，不重复执行输入预处理和模型推理。

测试结果汇总如下：

| 测试项 | 计时范围 | 实测结果 |
| --- | --- | --- |
| Benchmark | 模型前向推理一次，即 `select_action` | 361.958 ms |
...
- 先执行 `deactivate`，再激活 `source ~/.lerobot-venv/bin/activate`；
- 不通过 `examples/spine-torch-deploy/run_lerobot_fp16_a100x8.sh` 启动；
- `--policy.path` 指向原 `pretrained_model`；
- 直接使用标准 `lerobot-record` 入口。
