...
K3 使用 Bianbu v3+ 固件时，系统 Python 版本为 3.14。当前 LeRobot 使用的 Spacemit PyTorch wheel 基于 Python 3.12 构建，因此本方案使用 `uv` 安装 Python 3.12 并创建虚拟环境。更多平台说明参见[进迭时空 Python 开发文档](https://www.spacemit.com/community/document/info?lang=zh&nodepath=software/SDK/bianbu/development/python.md)，`uv` 的完整命令参见 [uv 官方文档](https://docs.astral.sh/uv/)。
...
2. 使用 Python 3.12 创建 LeRobot 虚拟环境，并通过 `--seed` 预装 pip：
...
1. 使用 `uv` 和 Python 3.12 创建 FP16 专用虚拟环境，并预装 pip：
