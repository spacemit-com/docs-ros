...
本文档介绍 ACT（Action Chunking Transformer）策略模型在 SpacemiT K3 开发板上的 ONNX 部署方法，覆盖模型导出、量化、Python 推理、C++ 推理、离线验证和 SO-101 真机运行。
...
如图是 Python 真机运行的效果，对比 Pytorch 版本，由于推理时延的大幅降低，动作快之间衔接流畅了很多。
