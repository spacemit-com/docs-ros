...
直接将整个 SmolVLA 图转成 fp16 后，部分视觉注意力、归一化和索引更新相关子图容易出现数值漂移。尤其是 `prefill_lm` 和 `denoise_step` 中的 RMSNorm 子图会参与语言模型和扩散去噪的关键计算，误差会在多步 denoise 中累积，最终可能表现为 action 异常、抓取失败甚至 fp16 溢出。因此需要在 fp16 模型上执行算子手术，保留主要算子的 fp16 加速，同时把敏感子图改成更稳定的形式。
