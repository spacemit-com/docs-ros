...
Python 真机入口依赖 LeRobot、Feetech 驱动、safetensors 和 SpaceMIT EP。复用 Python 3.12 虚拟环境 `~/.lerobot-venv`。若尚未安装 `uv`，先参考[《真机训练推理》](3.3.1-真机训练推理.md#33-uv-安装和使用)完成安装。

K3 运行环境不安装 `xslim`。`xslim` 依赖 `opencv-python`，与 LeRobot 使用的 `opencv-python-headless` 共享 `cv2` 目录并产生文件冲突。模型量化在 x86 主机执行。
...
`--seed` 会在新环境中预装 pip，`--allow-existing` 会保留现有环境中的软件包。`python --version` 应输出 Python 3.12。`python -m pip` 始终使用当前虚拟环境的 pip，避免 `pip3` 误用系统 Python 并触发 `externally-managed-environment`。OpenCV 软件包列表中只应包含 `opencv-python-headless`。
...
下载[基于《真机训练推理》导出和量化的模型](https://archive.spacemit.com/spacemit-ai/model_zoo/vla/act/onnx/models.tar.gz)，可复现本文的基准测试。
...
在 x86 主机使用 xslim 动态量化生成 INT8 模型：
...
SmolVLA ONNX 部署包含 FP32 导出、FP16 量化、FP16 图修复、数值对比和真机运行。K3 真机部署推荐使用 C++ FP16 图修复版模型；Python 路径用于 FP32 离线验证和调试。

### 4.1 环境搭建

K3 Python 推理和数值对比复用 Python 3.12 虚拟环境 `~/.lerobot-venv`。以下命令会通过 `uv` 补齐 pip，并保留环境中已经安装的软件包：

K3 运行环境不安装 `xslim`。SmolVLA 的 FP16 转换和图修复在 x86 主机执行。
...
`python --version` 应输出 Python 3.12。不要使用系统 `pip3`。OpenCV 软件包列表中只应包含 `opencv-python-headless`。
...
K3 上运行 FP16 图修复版模型时需要补丁版 SpacemiT ORT SDK：
...
> [!NOTE]
>
> SmolVLA FP16 图修复版需要在补丁版 SpacemiT ORT SDK 上运行。若使用系统 SpaceMIT EP、普通 ONNX Runtime 或 CPU 运行 FP16，可能出现数值漂移或溢出。Python 调试使用 FP32 模型。
...
直接将整个 SmolVLA 图转成 FP16 后，部分视觉注意力、归一化和索引更新相关子图容易出现数值漂移。尤其是 `prefill_lm` 和 `denoise_step` 中的 RMSNorm 子图会参与语言模型和扩散去噪的关键计算，误差会在多步 denoise 中累积，最终可能表现为 action 异常、抓取失败甚至 FP16 溢出。因此需要在 FP16 模型上执行图修复，保留主要算子的 FP16 加速，同时把敏感子图改成更稳定的形式。
...
- 保持 `connector` 等主要前向计算仍为 FP16，以保留 K3 上的推理速度收益。

图修复后的目录仍包含 4 个 ONNX 子图：
...
在 x86 主机上使用 CPU 对比 FP32 与 FP16 图修复版输出：
...
K3 上的 FP16 验证需要同时加载补丁版 `libonnxruntime.so` 和 `libspacemit_ep.so`，因此使用 4.8.2 节的 C++ EP204 数值检查作为 K3 验收步骤。不要在 Python 进程中通过 `--spacemit-ort-dir` 加载补丁版 SDK。
...
Python SmolVLA 路径使用 FP32 模型进行离线验证或 dry-run 调试。K3 上运行 FP32 模型时需要传入 `--cpu-denoise`：`denoise_step` 使用 CPU EP，其余 3 个子图使用 SpaceMIT EP。FP16 图修复版模型通过 C++ 路径部署，因为 FP16 数值正确性依赖补丁版 SpacemiT ORT SDK 和对应 EP 配置。
...
离线基准测试不连接机械臂和相机，使用合成 observation 运行完整 4-model pipeline。该模式用于验证模型加载、EP 是否可用，以及统计 full-inference 延迟。
...
| `--cpu-denoise` | FP32 `denoise_step` 使用 CPU，其余子图仍使用 SpaceMIT EP | 关闭 |
...
C++ 路径是 SmolVLA 在 K3 上的推荐部署方式。默认运行 `models/onnx/smolvla-fp16-surgeried`，并通过 `run_smolvla_robot_pipeline.sh` 使用补丁版 SpacemiT ORT SDK、EP 线程和绑核配置。
...
先使用合成输入运行无硬件检查。该命令不打开机械臂和相机，也不下发电机动作：
...
检查日志中 4 个 ONNX 子图均成功创建 SpaceMIT EP session，10 次 `denoise_step` 的 `bad` 计数均为 `0`。

#### 4.8.3 dry-run 安全检查

真机执行前先进行 dry-run。该模式会打开相机和机械臂、读取真实状态和图像，但不会下发动作：
...
dry-run 通过后，再进入基准测试或真机运行。

#### 4.8.4 完整推理基准测试

运行完整推理基准测试时打开 `--infer-every-tick`，避免 action queue 复用动作。该模式每个控制周期都会重新采集 observation 并执行完整 4-model pipeline，用于统计真实推理耗时。
...
- `--max-iters 3` 表示只运行 3 个控制周期，适合基准测试。
- `--dry-run` 禁止下发电机动作，但仍采集真实 observation 并执行完整推理。
...
| `--infer-every-tick` | 每个控制周期都完整推理，用于基准测试 | 关闭 |
| `--dry-run` | 只算动作，不下发电机 | 关闭 |
...
![SmolVLA ONNX 真机运行效果](assets/smolvla-onnx.gif)

## 5. K3 实测参考

延迟会随固件、ORT SDK、模型和散热状态变化。以下数据只用于估算量级，不作为当前版本的性能验收结果。
...
| `smolvla-fp32` | EP 8 线程 + CPU denoise | 8762.5 ms |
...
| `act-int8` | EP 8 线程 | 247.2 ms |
| `smolvla-fp32` | EP 8 线程 | 9023.9 ms |
| `smolvla-fp16` | EP204 8 线程 | 1113.2 ms |

SmolVLA 数据使用 2 路相机模型和 `--denoise-steps 10`，统计完整推理平均耗时，不包含模型加载。

使用仓库提供的 K3 性能门禁复测推荐路径：
...
ACT 脚本默认执行 3 次 warmup 和 20 次 INT8 测量，平均延迟阈值为 500 ms。SmolVLA 脚本使用合成输入、EP204、1 次 warmup 和 3 次 FP16 测量，平均延迟阈值为 1600 ms。两个脚本通过时都会输出 `PERF_OK`。

2026 年 8 月 10 日在 K3 复测时，ACT INT8 平均延迟为 247.226 ms，SmolVLA FP16 平均延迟为 1113.167 ms，均通过性能门禁。

## 6. 常见问题

### 6.1 ACT 和 SmolVLA 应该选择哪个部署入口？

ACT 推荐优先使用 Python 或 C++ INT8 模型，端到端延迟更低。SmolVLA 推荐使用 C++ FP16 图修复版模型，Python 路径主要用于 FP32 离线验证和 dry-run 调试。

### 6.2 SmolVLA FP16 为什么必须使用补丁版 SpacemiT ORT SDK？

SmolVLA FP16 对部分算子数值更敏感。补丁版 SDK 配合 FP16 图修复版模型验证过数值正确性；使用系统 SpaceMIT EP、普通 ONNX Runtime 或 CPU 运行 FP16，可能出现数值漂移或溢出。
...
### 6.6 K3 导入 `cv2` 时为什么提示 `gapi_wip_gst_GStreamerPipeline` 不存在？

该错误表示 K3 虚拟环境同时安装了 `opencv-python` 和 `opencv-python-headless`。两个发行包会向同一个 `cv2` 目录写入不同版本的 Python 文件和二进制扩展。

以下命令会卸载 K3 虚拟环境中的 `xslim` 和两个 OpenCV 发行包，然后重新安装 LeRobot 使用的 headless 版本。不要在 x86 导出和量化环境执行这组命令。
...
验证输出应显示 OpenCV 版本为 `4.12.0`、`VideoCapture` 为 `True`，并且软件包列表中只有 `opencv-python-headless`。
