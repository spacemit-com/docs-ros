...
参考 LeIsaac 文档 [add custom task](https://lightwheelai.github.io/leisaac/docs/tutorials/custom_task)，添加抓取任务资产和仿真场景。
...
> [!TIP]
>
> 1. 校准过程参考 [SO101 机械臂校准流程](https://huggingface.co/docs/lerobot/en/so101#calibrate)。
> 2. 校准文件存放于 `~/.cache/huggingface/lerobot/calibration` 目录下。
...
模型训练完成后，需将 checkpoint 目录传输到推理端 `leisaac/models` 目录。也可以下载笔者已训练好的 [模型权重](https://archive.spacemit.com/spacemit-ai/model_zoo/vla/act/act_test_custom_task_amp.tar.gz)，保持一致的仿真环境即可直接分布式推理。
...
- 服务器可正常启动仿真环境，`list_envs.py` 输出包含 `LeIsaac-SO101-CustomTask-v0` 抓取任务环境；
