...
Perceptive Grasp 是面向 Linksee 轮式机器人的感知抓取方案，用于完成桌面或近距离场景中的目标识别、空间定位、抓取规划和机械臂执行。系统以 D435i 深度相机作为感知输入，通过 YOLOv8-seg 完成目标检测，并结合深度反投影、手眼标定结果和顶抓策略，生成 SO101 机械臂可执行的抓取位姿。当目标超出机械臂舒适抓取区时，系统可触发底盘辅助对齐，停车后重新感知并规划抓取；同时通过本地 ASR/TTS 支持语音命令输入和任务状态播报，提升抓取任务的自主性和人机交互体验。

### 10.1 方案概述

感知抓取主链路如下图所示。底盘辅助在抓取规划阶段介入，当目标超出机械臂舒适抓取区时，底盘先执行一次短距离前进、后退或原地转向，停车稳定后重新检测目标并重新规划抓取。

![Linksee Perceptive Grasp 抓取链路](assets/linksee-perceptive-grasp-flow.svg)

语音链路使用本地语音桥连接音频 SDK 和抓取程序。ASR 方向负责把语音命令写入抓取程序的 `voice-stdin`，TTS 方向负责读取抓取程序的 `status-stdout` 并播报状态。

![Linksee Perceptive Grasp 语音链路](assets/linksee-perceptive-voice-flow.svg)
...
| 硬件 | 用途 |
| ---- | ---- |
| Linksee 轮式机器人 | 机器人本体，提供 K3 计算平台、差速底盘和机身结构 |
| SO101 机械臂和夹爪 | 执行顶抓、夹取、抬起和放置动作 |
| RealSense D435i 深度相机 | 提供 RGB-D 图像，用于目标检测、深度定位和抓取点规划 |
| 底盘 UART 串口 | 连接 Linksee 底盘控制板，执行短距离对齐动作 |
| USB Camera 麦克风 | 采集语音命令，供本地 VAD/ASR 识别 |
| USB Audio 扬声器 | 播报系统状态、失败原因和任务结果 |

实物连接如图所示：

<div style="display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px; align-items: start;">
  <img src="assets/perception_grasp_1.jpg" alt="Linksee 感知抓取硬件连接图 1" style="width: 100%; border-radius: 8px; display: block;" />
  <img src="assets/perception_grasp_2.jpg" alt="Linksee 感知抓取硬件连接图 2" style="width: 100%; border-radius: 8px; display: block;" />
</div>

左图展示了 D435i 深度相机面向机械臂工作区的安装方式，用于获取目标的 RGB-D 图像；右图展示了 K3 开发板通过 USB 接口连接机械臂串口、底盘串口、深度相机、麦克风和扬声器，分别完成执行控制、底盘辅助、视觉感知、语音输入和语音播报。
...
sudo apt install -y ros-humble-librealsense2 v4l-utils \
  cmake build-essential libeigen3-dev libboost-all-dev liburdfdom-dev
```

Pinocchio 提供 SO101 机械臂的 FK/IK 运动学求解能力，按 [机械臂控制文档](../05-机器人开发/5.2-运动控制/5.2.4-机械臂控制.md#31-示例一基础框架与运动学验证) 中的“Pinocchio 源码编译”步骤安装到 `/usr/local`。安装完成后执行：

```bash
sudo ldconfig
```

Python 虚拟环境用于运行环境检查、手眼标定和本地语音桥：
...
按下面顺序构建 SDK 组件。每一步都应生成对应头文件和动态库；前一步未构建成功时，不要继续执行后续步骤。

1. 构建底盘控制库，供 `mobile_base` 模块控制 Linksee 底盘短距离前进、后退和原地转向。
...
2. 构建视觉推理库，供检测模块调用 VisionService 和 YOLOv8-seg。
...
4. 构建机械臂控制库，供 SO101 机械臂和 Pinocchio IK 使用。
...
下载语音模型。SenseVoice 用于 ASR，Silero 用于 VAD，Matcha-TTS、Vocos 和文本归一化 FST 用于 TTS 播报：
...
检查脚本会检查串口、RealSense D435i、Pinocchio IK 后端、底盘驱动注册、语音 Python 包和音频设备。脚本还会列出 `/dev/ttyACM*` 对应的 `/dev/serial/by-id/*` 稳定路径，并给出机械臂和底盘串口建议，例如：
...
将建议路径写入 `config/grasp_pipeline.yaml`。部署时优先使用 `/dev/serial/by-id/...`，不要依赖 `/dev/ttyACM0/1/2` 这类会随插拔顺序变化的临时设备号。

```yaml
manipulator:
  uart_device: "/dev/serial/by-id/<检查脚本建议的机械臂串口>"

mobile_base:
  enabled: true
  driver: "drv_uart_esp32"
  dev_path: "/dev/serial/by-id/<检查脚本建议的底盘串口>"
  baud: 115200
```
...
如果脚本提示当前用户不在 `dialout`、`video` 或 `audio` 用户组，按提示修复后执行下面命令让当前终端重新加载用户组：

```bash
exec su - "$USER"
```

重新进入项目并再次运行检查，最终结果应为：
...
感知抓取使用 Eye-to-Hand 标定：D435i 固定在机身上，ChArUco 标定板固定在夹爪末端。相机安装位置变化、抓取偏移明显或抓取不稳定时，需要重新标定。
...
将 `config/charuco_4x5_20mm_14mm.png` 按 100% 比例打印，并固定在夹爪末端。打印后用尺子测量一个黑白方格边长；如果不是 20 mm，下面采集命令中的 `--charuco-square-length` 需要改成实测值，单位为米。
...
写回后重新运行环境检查，并确认 `config/grasp_pipeline.yaml` 中的 `calibration.T_base_camera` 已更新。
...
常用目标名称需与检测模型类别一致，例如 `banana`、`apple`、`orange`、`carrot`、`bottle`、`cup`、`bowl`、`spoon`、`fork` 和 `scissors`。指定目标模式执行一次抓取后会回到 Home 姿态并退出。

启用 `mobile_base.enabled=true` 后，如果目标在机械臂舒适抓取区外，日志中会进入 `BASE_ALIGNING` 状态。底盘每次只执行一个短动作，动作结束后重新检测目标并重新规划；默认最多调整 6 次。首次测试前确保机器人周围有安全空间，并保持 `linear_speed`、`angular_speed` 等参数为默认低速值。

#### 10.3.9 语音模式

启动本地语音桥。语音桥会自动启动抓取程序，并负责录音、VAD、ASR、TTS 和状态播报。
...
“抓香蕉”会启动一次香蕉抓取任务；“停止”会停止当前任务，并回到观察位等待下一条命令；“结束”“回家”会让机械臂回到 Home 姿态并退出程序。

调试命令解析时，可以跳过 ASR/TTS，直接启动文本输入模式，然后在终端输入“抓香蕉”等文本：
...
./read_joints --config ../config/grasp_pipeline.yaml
```

抓取过程中会在终端输出状态，并按配置保存调试文件。默认调试目录为 `debug_grasp_runs/`，其中包含抓取时的图像和 JSON 结果，可用于确认检测框、抓取像素、三维位置、底盘对齐动作和执行结果。

如图是语音控制抓取香蕉的结果：

![](assets/grasp-move.gif)
