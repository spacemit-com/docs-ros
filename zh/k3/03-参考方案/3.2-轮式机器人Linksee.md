...
perceptive grasp 是面向 Linksee 轮式机器人的近距离感知抓取应用。系统通过立体相机获取彩色图像和深度信息，使用 yolov8-seg 检测并分割目标，再结合深度反投影、eye-to-hand 手眼标定和轻量三维几何规划，生成机械臂可执行的顶抓或侧抓位姿。

系统由以下模块组成：

- 感知输入：`realsense d435i` 或 `spacemit_las2`，输出彩色图像、深度信息和相机内参。
- 目标检测：`spacemit_vision_service(yolov8-seg)`，输出目标类别、检测框和分割掩码。
- 三维估计：`grasp_geometry`，生成目标点云并估计桌面平面、三维尺寸和水平主轴。
- 闭环规划：`grasp_pipeline / grasp_planner`，完成目标稳定判断、策略锁定、底盘对齐、重感知和候选安全校验。
- 机械臂执行：`spacemit_manipulator/grasp`，控制 Linksee 机械臂和夹爪完成抓取、放置、归位和失败恢复。
- 底盘对齐：`spacemit_base`，调整机器人与目标的相对位置。
...
性能受目标几何、场景深度质量和 ik 候选数量影响：

- 所有目标都会经过检测稳定判断、目标点云生成、桌面平面估计和三维策略初判。
- 顶抓使用二维分割掩码和对齐深度确定最终抓取点，再验证预抓取、下降和抬升路径。
- 侧抓根据目标点云和桌面平面生成安全入场位、预抓取位、抓取位、抬升位和退出位，并验证完整关节路径。

以下数据统计自 k3 使用 realsense d435i 时的实机运行日志。每个有效轮次由一次成功的单帧检测及其后紧邻的规划组成，不包含目标稳定等待、机械臂动作、底盘动作、失败重试和超时轮次。发布配置使用 `stable_frames: 3`，因此用户实际观察到的完整检测阶段还包含连续帧稳定判断时间。

| 抓取策略 | 有效轮次 | 检测耗时 | 规划耗时 | 检测与规划合计 |
| --- | ---: | ---: | ---: | ---: |
| 顶抓 | 21 | 平均 131 ms（125～143 ms） | 平均 100 ms（84～211 ms） | 平均 231 ms（215～344 ms） |
| 侧抓 | 9 | 平均 128 ms（125～132 ms） | 平均 115 ms（84～165 ms） | 平均 243 ms（209～296 ms） |
