...
本章节介绍 Linksee 机器人的感知抓取方案。Perceptive Grasp 通过 RealSense D435i 获取 RGB-D 图像，使用 YOLOv8-seg 完成目标检测，结合深度定位和顶抓规划生成抓取位姿，并控制 SO101 机械臂和夹爪完成抓取。系统同时支持本地 ASR/TTS 语音交互，可通过“抓香蕉”“停止”“结束”等语音命令触发和控制抓取流程。

### 10.1 方案概述

感知抓取由视觉抓取主链路和本地语音控制链路组成。视觉链路负责识别目标、估计空间位置并执行抓取；语音链路负责采集用户指令、解析目标名称和播报状态。总体通路如图所示：

![](assets/linksee-perceptive-grasp-flow.png)
...
左图展示了 D435i 俯视机械臂工作区的安装方式；右图展示了深度相机、麦克风和扬声器的连接方式。
...
先安装 Pinocchio。Pinocchio 提供 SO101 机械臂的 FK/IK 运动学求解能力，按 [机械臂控制文档](../05-机器人开发/5.2-运动控制/5.2.4-机械臂控制.md#31-示例一基础框架与运动学验证) 中的“Pinocchio 源码编译”步骤安装到 `/usr/local`，安装完成后执行：

```bash
sudo ldconfig
```

随后按顺序构建视觉、抓取和机械臂组件。前一个组件未构建成功时，不要继续执行后续步骤。

```bash
cd ~/spacemit_robot/components/model_zoo/vision
mm

cd ~/spacemit_robot/components/control/control_grasp
mm

cd ~/spacemit_robot/components/control/control_manipulator
mm
```

确认 SDK 产物已安装到 `output/staging`。
...
检查脚本会检查串口、相机、Pinocchio IK 后端、语音 Python 包和音频设备。需要补权限或安装依赖时，脚本会弹出 `y/N` 确认并执行修复命令。如果脚本此前未加入 `dialout`、`video` 或 `audio` 用户组，执行下面命令让当前终端重新加载用户组：
...
“抓香蕉”会启动一次香蕉抓取任务；“停止”会停止当前任务，并回到观察位等待下一条命令；“结束”会让机械臂回到 Home 姿态并退出程序。
...
![](assets/perception_grasp.gif)

抓取过程中会在终端输出状态，并按配置保存调试文件。默认调试目录为 `debug_grasp_runs/`，其中包含抓取时的图像和 JSON 结果，可用于确认检测框、抓取像素、三维位置和执行结果。
