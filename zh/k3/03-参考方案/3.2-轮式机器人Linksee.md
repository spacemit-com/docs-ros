...
perceptive grasp 是面向 Linksee 轮式机器人的近距离感知抓取应用，提供以下能力：

- 通过立体相机和 yolov8-seg 完成目标分割与三维定位。
- 结合 eye-to-hand 标定和顶抓规划生成机械臂可执行的抓取位姿。
- 在目标超出舒适抓取区时控制底盘辅助对齐，并在停车后重新感知和规划。
...
2. 目标检测和抓取规划完成目标定位与可达性判断；
3. 机械臂执行接近、夹取、抬升、放置和归位动作；
4. 目标不在舒适抓取区时，底盘在规划阶段介入，并在每次动作后重新检测目标；
5. 本地语音桥接收 asr 命令，并通过 tts 播报 pipeline 状态。
...
- 主流程为 `idle -> observing -> detecting -> planning -> approaching -> grasping -> lifting -> placing -> homing -> done`。
- 目标超出舒适区时，`planning` 转入 `base_aligning`，底盘停车后返回 `detecting` 重新检测和规划。
- 夹爪在闭合后和抬升后分别检查位置与负载，用于识别抓空或物体脱落。
...
以下数据来自 k3 开发板的实机运行日志，用于评估 grasp pipeline 的阶段耗时。

测试条件如下：

- 立体相机后端为 `spacemit_las2`，深度推理使用一个会话并绑定核 `8`。
- 目标检测使用 `yolov8-seg` 和 spacemit ep。
- `stable_frames` 设为 `1`，每轮检测获取一组彩色图像和深度信息。

关键阶段耗时如下：

| 模块或阶段 | 参考耗时 | 主要处理 |
| --- | ---: | --- |
| 初始化 | 3.76～3.86 s | 初始化立体相机、目标检测、机械臂和底盘模块，并完成深度推理和检测模型预热 |
| `spacemit_las2` 取帧与深度推理 | 约 114 ms/轮 | 获取彩色图像，执行双目深度推理并输出对齐的深度信息 |
| `yolov8-seg` 目标检测 | 26～31 ms/轮 | 根据彩色图像输出目标类别、检测框和分割掩码 |
| `detecting` 核心处理 | 141～146 ms/轮 | `spacemit_las2` 与 `yolov8-seg` 串行执行的合计耗时 |
| `planning` 核心处理 | 约 14 ms/轮 | 完成抓取像素选择、深度处理、三维反投影、手眼坐标变换、抓取位姿生成和工作空间检查 |
