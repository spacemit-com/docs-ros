...
perceptive grasp 是面向 Linksee 轮式机器人的近距离感知抓取应用。系统通过立体相机获取彩色图像和深度信息，使用 yolov8-seg 完成目标检测与分割，并结合深度反投影、eye-to-hand 手眼标定和轻量三维几何规划，生成机械臂可执行的顶抓或侧抓位姿。

perceptive grasp 提供以下能力：
...
- 在目标超出舒适抓取区时控制底盘短距离对齐，并在停车后重新感知和规划。
- 通过本地 asr 接收语音命令，使用 tts 播报任务状态，并根据音频硬件选择硬件或软件回声消除。
...
1. 立体相机输出像素对齐的彩色图像和深度信息。
2. yolov8-seg 输出目标类别、检测框和分割掩码。
3. pipeline 根据目标类别、分割掩码、深度和三维几何选择顶抓或侧抓策略。
4. 机械臂执行接近、夹取、抬升或退出、放置和归位动作。
5. 目标不在舒适抓取区时，底盘在规划阶段介入，并在每次动作后重新检测目标。
6. 本地语音桥接收 asr 命令，并通过 tts 播报 pipeline 状态。

![Linksee 感知抓取链路](assets/grasp-system-flow.svg)

总体来说，系统由以下模块组成：

- 立体相机：`realsense d435i` 或 `spacemit_las2`，输出彩色图像和深度信息。
...
- 底盘辅助：`spacemit_chassis`，调整机器人与目标的相对位置。
- 语音交互：`spacemit_audio/vad/asr/tts`，接收语音命令、播报状态并处理播放回声。
...
- 立体相机，支持 realsense d435i 深度相机或 spacemit_las2 双目相机。
- 麦克风和扬声器，启用语音交互时需要。
...
- 左图展示立体相机面向机械臂工作区的安装方式。
- 右图展示 k3 与立体相机、机械臂、底盘和音频设备的连接关系。
...
应用只初始化 `camera.type` 选中的后端。cmake 自动探测本机已安装的所有后端。运行时选择的后端必须包含在构建摘要中。
...
主配置文件为 `config/grasp_pipeline.yaml`。首次运行环境检查前完成以下配置：

- 通过 `camera.type` 选择立体相机后端。
...
- `stable_frames` 表示进入规划前需要连续确认的检测帧数。
...
- `ENABLE_WEBRTC_AEC`：控制是否构建 webrtc 软件回声消除模块，默认开启。使用 `hardware_aec` 或 `half_duplex` 时可设置为 `OFF`。
- `ENABLE_ROS2_VOICE`：控制是否构建基于 ros2 `std_msgs/String` 的语音话题桥，默认关闭。本节使用的本地语音桥不依赖该选项。
...
`grasp.strategy` 支持 `auto`、`top` 和 `side`。发布配置使用 `auto`：普通目标使用顶抓，杯、瓶根据三维几何选择顶抓或侧抓。
...
- `top` 参数只影响顶抓，`side` 参数只影响侧抓。
- `geometry` 控制目标点云、桌面平面、夹爪宽度和规划耗时约束。
...
- `mixer_volume`：启动时设置的 alsa `pcm` 音量，设为 `-1` 时不修改 mixer。
...
perceptive grasp 使用 eye-to-hand 标定计算立体相机坐标系到机械臂基座坐标系的变换。安装如图所示，要求如下：

- 立体相机固定在机身上。
- charuco 标定板刚性固定在夹爪末端。

<p align="center">
  <img src="assets/eye-to-hand.jpg" alt="立体相机与 charuco 标定板安装" width="480">
</p>
...
脚本会更新 `calibration.T_base_camera` 并备份原配置。
...
真实运动前清理机械臂工作区，并为底盘前后移动和转向预留安全空间。
...
- 主流程为 `idle -> detecting -> planning -> observing -> detecting -> planning -> approaching -> grasping -> lifting -> placing -> homing -> done`。
- 首次检测和规划只负责锁定抓取策略。普通目标使用顶抓，杯、瓶根据三维几何选择顶抓或侧抓。
- 顶抓和侧抓分别进入 `observe_joints` 和 `side_ready_joints`，到达观察位后重新检测目标并规划。
- 目标超出舒适区时，`planning` 转入 `base_aligning`。底盘完成一次短动作并停车后，pipeline 返回 `detecting`，不复用移动前的目标坐标。
- 顶抓从目标上方向下接近。侧抓先移动到目标上方的安全预抓取位，张开夹爪后下降并水平进给，再闭合夹爪。
- 夹爪在闭合后和抬升后检查位置与负载，用于识别抓空或物体脱落。
- 任一运行阶段失败时，pipeline 进入 `recovering`。单次任务回到 home 位；夹爪可能持物时也会优先回到 home 位并停止自动循环。
...
`--loop` 不限制循环次数。每轮完成后，机械臂回到本轮策略对应的观察位，并重新检测目标。按一次 `Ctrl+C` 后，程序完成当前动作并回到 home 位；仅在无法安全退出时再次按 `Ctrl+C` 强制终止。
...
```text
[VoiceBridge] Echo cancellation: mode=webrtc_aec
...
性能受相机后端、目标类别、场景深度质量和 ik 候选数量影响：

- 普通目标使用二维分割掩码和对齐深度生成顶抓位姿。
- 杯、瓶还会执行目标点云生成、桌面平面估计、三维尺寸计算和顶抓或侧抓候选筛选。
- spacemit_las2 每次取帧时执行一次深度推理；发布配置使用一个会话并绑定 x100 ai 逻辑核 `8`，其余 ai 核用于 yolov8-seg。

以下数据来自 k3 实机日志。测试使用 yolov8-seg 和 `stable_frames: 1`，统计到达对应观察位后的成功检测与规划轮次，不包含机械臂动作、底盘动作、重试和超时轮次。

| 立体相机后端 | 抓取策略 | 有效轮次 | 检测耗时 | 规划耗时 | 检测与规划合计 |
| --- | --- | ---: | ---: | ---: | ---: |
| realsense d435i | 顶抓 | 1 | 131 ms | 91 ms | 222 ms |
| realsense d435i | 侧抓 | 6 | 平均 126 ms（125～126 ms） | 平均 116 ms（104～159 ms） | 平均 242 ms（229～285 ms） |
| spacemit_las2 | 顶抓 | 2 | 196 ms | 平均 62 ms（56～68 ms） | 平均 258 ms（252～264 ms） |
| spacemit_las2 | 侧抓 | 7 | 平均 171 ms（170～173 ms） | 平均 133 ms（132～136 ms） | 平均 305 ms（302～308 ms） |
...
![](assets/perception_grasp_las2.gif)
