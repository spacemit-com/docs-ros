...
如果客户不下载 Robot SDK，只需要在系统中运行或编译自己的 ROS 2 Humble 功能包，可按本节准备 ROS 2、系统构建依赖和 Python 源配置。若使用 Robot SDK 主流程，可跳过本节，继续执行第 2 节。

在 K3 推荐镜像中，ROS 2 Humble 相关软件包可直接通过系统软件源安装。先更新包索引，再安装 ROS 2 Humble 及常用开发工具：
...
sudo apt install \
  ...
  libzmq3-dev libssl-dev libusb-1.0-0-dev libudev-dev \
  libgtk-3-dev libglfw3-dev libgl1-mesa-dev libglu1-mesa-dev \
  librange-v3-dev v4l-utils
```

安装 Python 基础环境，并配置 PyPI 源。阿里云源作为默认源，进迭时空 PyPI 源作为额外源，用于获取 RISC-V 平台适配的 Python 包：
